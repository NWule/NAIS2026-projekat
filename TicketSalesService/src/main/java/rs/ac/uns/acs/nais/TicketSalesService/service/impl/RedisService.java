...
public class RedisService implements IRedisService {
    ...
    private static final long SEAT_RESERVATION_TTL_SECONDS = 600;
    private static final long TICKET_PRICE_TTL_SECONDS = 300;
    private static final long EVENT_DATA_TTL_SECONDS = 600;
    ...
    private String seatKey(String eventId, String seatId) {
        return "seat:reservation:" + eventId + ":" + seatId;
    }

    private String priceKey(String eventId, String zoneId) {
        return "ticket:price:" + eventId + ":" + zoneId;
    }

    private String eventKey(String eventId) {
        return "event:cache:" + eventId;
    }

    @Override
    public boolean reserveSeat(String eventId, String seatId, String customerId) {
        String key = seatKey(eventId, seatId);
        Boolean set = redisTemplate.opsForValue()
                .setIfAbsent(key, customerId, SEAT_RESERVATION_TTL_SECONDS, TimeUnit.SECONDS);
        return Boolean.TRUE.equals(set);
    }

    @Override
    public Optional<String> getSeatReservation(String eventId, String seatId) {
        String value = redisTemplate.opsForValue().get(seatKey(eventId, seatId));
    ...
    public void cacheTicketPrice(String eventId, String zoneId, Double price) {
        redisTemplate.opsForValue().set(
                priceKey(eventId, zoneId),
                String.valueOf(price),
                TICKET_PRICE_TTL_SECONDS, TimeUnit.SECONDS
        );
    }
    ...
    public void cacheEventData(String eventId, String eventJson) {
        redisTemplate.opsForValue().set(
                eventKey(eventId),
                eventJson,
                EVENT_DATA_TTL_SECONDS, TimeUnit.SECONDS
        );
    }
    ...
    @Override
    public void invalidateEventData(String eventId) {
    ...
    public Map<String, Object> getCacheStats() {
        ...
        if (reservationKeys != null) {
            stats.put("reservationKeys", reservationKeys);
        }
        return stats;
    }
}
