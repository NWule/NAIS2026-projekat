...
public class EventElasticController {
    ...
    @PostMapping
    public ResponseEntity<EventDocument> create(@RequestBody EventDocument document) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<EventDocument> update(@PathVariable String id,
                                                @RequestBody EventDocument document) {
    ...
    public List<EventDocument> findByEventType(@PathVariable String eventType) {
        return service.findByEventType(eventType);
    }

    @GetMapping("/status/{status}")
    public List<EventDocument> findByStatus(@PathVariable String status) {
        return service.findByStatus(status);
    }

    @GetMapping("/city/{city}")
    public List<EventDocument> findByCity(@PathVariable String city) {
        return service.findByCity(city);
    }

    @GetMapping("/price-range")
    public List<EventDocument> findByPriceRange(@RequestParam Double minPrice,
                                                @RequestParam Double maxPrice) {
        return service.findByPriceBetween(minPrice, maxPrice);
    }

    // Složeni upit 1 — full-text pretraga sa filterima, sortiranjem i paginacijom
    @GetMapping("/search")
