...
@RestController
@RequestMapping("/elastic/ticket-sales")
public class TicketSaleElasticController {
    ...
    @PostMapping
    public ResponseEntity<TicketSaleDocument> create(@RequestBody TicketSaleDocument document) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<TicketSaleDocument> update(@PathVariable String id,
                                                     @RequestBody TicketSaleDocument document) {
    ...
    public List<TicketSaleDocument> findByCustomer(@PathVariable String customerId) {
        return service.findByCustomerId(customerId);
    }

    @GetMapping("/event/{eventId}")
    public List<TicketSaleDocument> findByEvent(@PathVariable String eventId) {
        return service.findByEventId(eventId);
    }

    @GetMapping("/event-type/{eventType}")
    public List<TicketSaleDocument> findByEventType(@PathVariable String eventType) {
        return service.findByEventType(eventType);
    }

    @GetMapping("/payment-method/{paymentMethod}")
    public List<TicketSaleDocument> findByPaymentMethod(@PathVariable String paymentMethod) {
        return service.findByPaymentMethod(paymentMethod);
    }

    @GetMapping("/price-range")
    public List<TicketSaleDocument> findByPriceRange(@RequestParam Double minPrice,
                                                     @RequestParam Double maxPrice) {
        return service.findByPriceBetween(minPrice, maxPrice);
    }

    // Složeni upit 2 — analitika po tipu dogadjaja i načinu plaćanja
    ...
    public List<Map<String, Object>> getSalesAnalytics(
            ...
            @RequestParam(defaultValue = "2030-12-31") String endDate) {
        return service.getSalesAnalyticsByEventTypeAndPaymentMethod(startDate, endDate);
    }

    // Složeni upit 3 — top dogadjaji po prihodu
