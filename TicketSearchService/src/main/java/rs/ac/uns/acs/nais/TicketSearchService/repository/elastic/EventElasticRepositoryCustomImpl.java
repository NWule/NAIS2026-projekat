    ...
    public Map<String, Object> searchEventsFullText(String searchText, String eventType, String status,
        ...
        try {
            ...
            SearchResponse<EventDocument> response = elasticsearchClient.search(s -> {
                var builder = s.index("events").from(page * size).size(size)
                    .sort(so -> so.field(f -> f.field(sortField).order(order)));

                builder.query(q -> q.bool(b -> {
                    if (searchText != null && !searchText.isBlank()) {
                        b.must(m -> m.multiMatch(mm -> mm.query(searchText).fields("name^3", "venue^2", "description", "organizer")));
                    } else {
                        b.must(m -> m.matchAll(ma -> ma));
                    }
                    if (eventType != null && !eventType.isBlank()) b.filter(f -> f.term(t -> t.field("eventType").value(eventType)));
                    if (status != null && !status.isBlank()) b.filter(f -> f.term(t -> t.field("status").value(status)));
                    if (minPrice != null) b.filter(f -> f.range(r -> r.field("basePrice").gte(JsonData.of(minPrice))));
                    if (maxPrice != null) b.filter(f -> f.range(r -> r.field("basePrice").lte(JsonData.of(maxPrice))));
                    return b;
                }));
            ...
            for (StringTermsBucket bucket : response.aggregations().get("by_competition").sterms().buckets().array()) {
                byCompetition.add(Map.of(
                    "competition", bucket.key().stringValue(),
                    "matchCount", bucket.docCount(),
                    "avgPrice", bucket.aggregations().get("avg_price").avg().value(),
                    "totalCapacity", bucket.aggregations().get("total_capacity").sum().value()
                ));
            }
            ...
            for (StringTermsBucket bucket : response.aggregations().get("by_status").sterms().buckets().array()) {
                byStatus.add(Map.of(
                    "status", bucket.key().stringValue(),
                    "count", bucket.docCount(),
                    "availableSeatsSum", bucket.aggregations().get("available_seats_sum").sum().value()
                ));
            }
